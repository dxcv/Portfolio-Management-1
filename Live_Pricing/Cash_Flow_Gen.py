...
@author: shaol
"""
import swap.Swap_Tools  as Tools
...
#    if r_type.upper() == "FIX":
#        """ Special case with fixed leg
#            especially when day counting
#            issue arise: 
#            quart_yearfrac + quart_yearfrac != semi_yearfrac
#        """
#        bal_tb = cftb_resample(bal_tb, pay_freq)
...
def INT_resample( cash_flow, 
    ...
    for idx, ele in enumerate(cash_flow):
        if idx > pay_freq[loc_ind][0]:
            ...
            """
            loc_ind += 1
        if Tools.last_day_of_month(ele["End_Time"]) == next_pay_date or idx >= len(cash_flow)-1:
            ...
            int_accru = 0
            cur_fwd_p = get_fwd_month(pay_freq[loc_ind][1]) 
            next_pay_date = ele["End_Time"] + relativedelta(months = cur_fwd_p)
            next_pay_date = Tools.last_day_of_month(next_pay_date)
          
        elif Tools.last_day_of_month(ele["End_Time"]) > next_pay_date or idx >= len(cash_flow)-1:
            temp = OrderedDict({ "Beg_Time":    ele["Beg_Time"],
                                 "End_Time":    ele["End_Time"], 
                                 "Beg_balance": ele["Beg_balance"],
                                 "PMT":         ele["Principal"]+int_accru,
        ...
        elif Tools.last_day_of_month(ele["End_Time"]) < next_pay_date:
            int_accru += ele["Interests"]
            temp = OrderedDict({ "Beg_Time":    ele["Beg_Time"],
                                 "End_Time":    ele["End_Time"], 
                                 "Beg_balance": ele["Beg_balance"],
