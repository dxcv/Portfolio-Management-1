...
import special.special_tools  as Tools
import special.Day_Counter_func as Day_Count
from special.Cash_Flow_Gen import CF_Gen as CF_Gen
...
class SPer:
    ...
    def add( self, 
             ...
             instrument, 
             disc_cv_details ):
        instr,itype = self.gen_sp_instruments( instrument )
        self.instruments[name] = [instr,disc_cv_details,itype]
    ...
    def cal_sp( self, 
        ...
        if len(self.instruments.items()) == 0:
            print("Get No Special Positions At ALL...")
            print("Return 0...")
            return 0
        for name, data in self.instruments.items():
            val_ans,cf_tb,delta_cf,fv_cf = self.cal_npv_sp( value_date, 
                                                            data[0], 
                                                            data[1],
                                                            data[2] )
            risk_ans = self.cal_risk_sp( value_date,
                                         data[0],
                                         data[1],
                                         data[2] )
            self.answer_bk[name] = {}
            self.answer_bk[name]["Type"] = data[2]
    ...
    def cal_value_single( self,
        ...
        cv_dis = self.gen_sp_curve( currency, 
                                      -1, 
                                      0,
                                      spread,
                                      Day_Counter,
        ...
        """
        cv_del = self.gen_sp_curve( currency, 
                                    0, 
                                    100,
                                    spread,
                                    Day_Counter,
    ...
    def gen_sp_instruments( self,
                            instrument ):
        """ generate special instrument from 
            general instrument
        """
        sp_instrument = OrderedDict()
        leg1 = OrderedDict()
        ...
        sp_instrument["leg1"] = leg1
        sp_instrument["leg2"] = leg2
        return sp_instrument, instrument["itype"]
    ...
    def to_string( self ):
        ...
        key_str = "Special Info:\n"
        key_str += "Current has: "+str(num) +" of Special.\n"
        key_str += "They are:\n"
